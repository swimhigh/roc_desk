...
**首页/多工作模式重新设计（2026-09-04 已实现）**：本节描述的"首页=会话树+工作区选择器常驻同屏"这个决策已被推翻重做——SSH 桌面、工作区（VS Code 式）、编辑器（UltraEdit 式）、资源管理器（Total Commander 式）现在是四个平等的"工作模块"，首页是一个分区仪表盘，点一个模块另开一个独立进程/窗口（不是本节原来设想的"单进程内切换"，也不是"分目录运行、按目录记忆模式"），可以同时开多个、同一模块也能开多个窗口。`HomeShell.tsx`（本节描述的组件）现在是 `ssh` 模块自己独立进程的内容，右侧内嵌的工作区选择器已经拆到 `workspace` 模块窗口。完整方案见 [docs/HOME_MODES_DESIGN.md](docs/HOME_MODES_DESIGN.md)，本节内容保留作为这次重做之前的历史决策记录。
