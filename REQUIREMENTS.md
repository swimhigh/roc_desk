...
- **RDP 远程桌面**（用户原话，方向经过三次收窄——"能改成和MobaXterm一样就改成和它一样的搞法"、"直接上 ActiveX 完整方案"）：最终方案是以 ActiveX 就地激活方式承载 Windows 自带的 RDP 客户端控件（`mstscax.dll`），与 `mstsc.exe`/MobaXterm 内部用的是同一个控件，不自己实现协议。踩坑与技术细节见 DESIGN.md §3.9.1、代码见 `src-tauri/src/rdp/`（`mstscax.rs` 手抄 COM 接口、`ole_container.rs` 最小 OLE 容器、`mod.rs` 会话生命周期）。**已知缺口**：未实现 `IMsTscAxEvents` 事件下沉，前端拿不到主动的"连接是否成功"回调，连接状态（含证书警告）完全交给控件自己的原生 UI；未在真实 RDP 服务器上做过人工验证（仅编译与单元测试通过）——这是本会话结束时的交接状态，下一次应优先做的是真机测试。
- **RDP 空白状态修复**（2026-08-25）：真机联调过程中依次修复了三个具体问题——①前端”标签切走/切回才显示”的 effect 只在 `visible` 变化时触发，新开标签从一开始就是 visible，导致内嵌窗口初次连接后从未真正被 `rdp_show` 摆到位/显示出来（控件激活成功、不报错，但界面空白），改成连接成功那一刻主动补一次 `show`；②新增 `rdp_status` 命令和 STA 线程状态探测（每 250ms 轮询 `Connected`/`ExtendedDisconnectReason`），不再把”COM 控件创建成功”误显示成”RDP 已连接”，连接失败/断开时显示错误码；③把 `IMsRdpClientAdvancedSettings2::AuthenticationLevel` 设成 0，避开”没有事件下沉、控件无法应答自签名证书确认请求而静默卡死在黑屏”这条路径（这台测试服务器已知会触发证书确认流程）。仍未实现完整 `IMsTscAxEvents` 事件下沉。
