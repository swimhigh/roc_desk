...
- **代码编辑**：基于 Monaco Editor，多标签同时编辑、语法高亮、搜索替换、Markdown/HTML 预览；支持 UTF-8/GBK/UTF-16 等文本编码自动探测与手动切换（编辑器底部状态栏显示当前编码，可"重新打开为"/"保存为"指定编码）；支持工作区内文本文件对比（右键文件"选择进行比较" → 右键另一个文件"与所选文件比较"，Monaco 并排差异视图）。
